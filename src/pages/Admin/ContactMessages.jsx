...
export default function ContactMessages() {
  ...
  // Fetch messages, refetch every 8 seconds for near realtime
  ...
  const statusColor = (s) => {
    ...
    if (s === 'sent' || s === 'read' || s === 'responded') return 'success';
                ...
                <TableRow key={m._id}>
                  ...
                  <TableCell sx={{ maxWidth: 480 }}>
                    ...
                    <Stack direction="row" spacing={1} sx={{ mt: 1 }}>
                      <Button size="small" onClick={() => navigator.clipboard?.writeText(m.message)}>Copy</Button>
                      <Button size="small" onClick={() => alert(m.message)}>View</Button>
                    </Stack>
                  </TableCell>
                    ...
                    <Stack direction="row" spacing={1} justifyContent="flex-end">
                      {m.status !== 'read' && (
                        <Tooltip title="Mark as read">
                          <IconButton onClick={() => handleMark(m._id, 'read')} size="small">
                            <MarkEmailReadIcon />
                          </IconButton>
                        </Tooltip>
                      )}
                      ...
                      {m.status === 'failed' && (
                        <Tooltip title="Retry send">
                          <IconButton onClick={() => handleMark(m._id, 'pending')} size="small">
                            <AutorenewIcon />
                          </IconButton>
                        </Tooltip>
                      )}
                      ...
                      <Tooltip title="Mark as responded">
                        <IconButton onClick={() => handleMark(m._id, 'responded')} size="small">
                          <ReplyIcon />
                        </IconButton>
                      </Tooltip>

                      <Tooltip title="Delete">
                        <IconButton onClick={() => handleDelete(m._id)} size="small" color="error">
                          <DeleteIcon />
                        </IconButton>
                      </Tooltip>
                    </Stack>
