...
export const AuthProvider = ({ children }) => {
  ...
  const decodeAndSetUser = useCallback(
    (jwtToken) => {
      ...
      setCurrentUser({ id: userId || id, email, role });
      setClaims({ role, orgs, permissions: permissions || [] });
      setToken(jwtToken);
      try {
  ...
  const hasPermission = useCallback((perm) => Array.isArray(claims?.permissions) && claims.permissions.includes(perm), [claims]);
  ...
  const value = useMemo(
    ...
    [currentUser, claims, token, isLoading, login, logout, register, refreshAccessToken, isAdmin, isAssessor, isCandidate, getOrgRole, hasPermission]
  );
