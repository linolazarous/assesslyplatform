...
if (process.env.NODE_ENV === 'production' && !process.env.REACT_APP_API_URL) {
  console.warn('REACT_APP_API_URL is not set in production environment');
}
    ...
    if (error.response) {
      ...
      if (status === 401) {
        ...
        if (!isLoginPage) {
          ...
          window.location.href = '/login?session=expired&reason=unauthorized';
        }
      }
 ...
 */
api.upload = async (endpoint, file, data = {}, onProgress = null, config = {}) => {
 ...
 */
api.download = async (endpoint, params = {}, filename = 'download', config = {}) => {
 ...
 */
api.retry = async (requestFn, maxRetries = 3, baseDelay = 1000) => {
