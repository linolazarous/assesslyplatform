...
submissionSchema.index({ organization: 1, assessment: 1 });
submissionSchema.index({ organization: 1, candidate: 1 });
submissionSchema.index({ organization: 1, status: 1 });
submissionSchema.index({ organization: 1, submittedAt: -1 });
...
// Performance indexes
submissionSchema.index({ organization: 1, createdAt: -1 });
submissionSchema.index({ organization: 1, reviewedBy: 1 });
submissionSchema.index({ organization: 1, passed: 1 });
submissionSchema.index({ organization: 1, percentage: -1 });

// Analytics indexes
submissionSchema.index({ organization: 1, 'metadata.proctoring.enabled': 1 });
submissionSchema.index({ organization: 1, 'metadata.violations.type': 1 });

// Time-based queries
submissionSchema.index({ organization: 1, expiresAt: 1 });
submissionSchema.index({ organization: 1, lastActivityAt: -1 });

// Review workflow
submissionSchema.index({ organization: 1, reviewStatus: 1 });
...
submissionSchema.virtual('isTimedOut').get(function() {
  return this.expiresAt && new Date() > this.expiresAt && this.status === 'in-progress';
});

submissionSchema.virtual('isUnderReview').get(function() {
  return this.status === 'under-review' || this.reviewStatus === 'in-progress';
});

submissionSchema.virtual('needsAttention').get(function() {
  return this.status === 'flagged' ||
         this.status === 'needs-clarification' ||
         this.reviewStatus === 'escalated' ||
         this.metadata.violations.some(v => !v.resolved);
});
...
submissionSchema.methods.startAttempt = function() {
...
submissionSchema.methods.submitAnswer = function(questionId, answerData) {
...
submissionSchema.methods.addViolation = function(violationData) {
  this.metadata.violations.push({
    ...violationData,
    timestamp: new Date()
  });

  // Auto-flag submission if critical violations
...
submissionSchema.methods.resolveViolation = function(violationId, resolvedBy, notes = '') {
  const violation = this.metadata.violations.id(violationId);
  if (violation) {
    ...
    if (notes) {
      ...
      violation.details.resolutionNotes = notes;
    }
  }

  // Check if all violations are resolved
  const unresolvedViolations = this.metadata.violations.filter(v => !v.resolved);
  if (unresolvedViolations.length === 0 && this.status === 'flagged') {
...
submissionSchema.methods.addReviewNote = function(note, createdBy, type = 'comment') {
...
submissionSchema.methods.startReview = function(reviewedBy) {
  this.status = 'under-review';
  this.reviewStatus = 'in-progress';
...
submissionSchema.methods.completeReview = function(feedback) {
...
submissionSchema.statics.findByOrganization = function(organizationId, query = {}) {
  return this.find({ organization: organizationId, ...query })
    ...
    .populate('metadata.violations.resolvedBy', 'name email');
};

submissionSchema.statics.getCandidateSubmissions = function(organizationId, candidateId) {
  return this.find({
...
submissionSchema.statics.getAssessmentStatistics = function(organizationId, assessmentId) {
  return this.aggregate([
    ...
    },
    {
      $group: {
        ...
        stdDevScore: { $stdDevPop: '$percentage' }
      }
    }
  ]);
};

submissionSchema.statics.getPendingReviews = function(organizationId) {
  return this.find({
    organization: organizationId,
    status: { $in: ['under-review', 'flagged', 'needs-clarification'] }
  })
...
submissionSchema.statics.getViolationReport = function(organizationId, days = 30) {
  const dateFilter = {
    submittedAt: {
  ...
  return this.aggregate([
    ...
    {
      $unwind: '$metadata.violations'
    },
    {
      $group: {
        _id: '$metadata.violations.type',
        count: { $sum: 1 },
        severityBreakdown: {
          $push: '$metadata.violations.severity'
        },
        resolvedCount: {
          $sum: {
            $cond: ['$metadata.violations.resolved', 1, 0]
          }
        }
      }
    }
...
submissionSchema.pre('save', function(next) {
  ...
  // Auto-expire submissions that are past their expiry
  if (this.expiresAt && new Date() > this.expiresAt && this.status === 'in-progress') {
    this.status = 'expired';
  }
...
submissionSchema.post('save', function(error, doc, next) {
  if (error.name === 'MongoError' && error.code === 11000) {
    next(new Error('A submission for this assessment attempt already exists.'));
  } else {
    next(error);
  }
});
