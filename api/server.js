...
app.get("/api/user/profile", authenticateToken, securityHeaders, async (req, res) => {
  try {
    ...
    res.json({ user, code: "PROFILE_RETRIEVED" });
  } catch (error) {
    console.error("Profile error:", error);
    res.status(500).json({ error: "Failed to retrieve profile", code: "PROFILE_RETRIEVAL_FAILED" });
  }
});
...
app.post("/api/organizations", authenticateToken, securityHeaders, async (req, res) => {
  try {
    ...
    if (!name) {
      return res.status(400).json({ error: "Organization name is required", code: "MISSING_NAME" });
    }
    ...
    res.status(201).json({ organization, code: "ORGANIZATION_CREATED" });
  } catch (error) {
    console.error("Organization creation error:", error);
    res.status(500).json({ error: "Failed to create organization", code: "ORGANIZATION_CREATION_FAILED" });
  }
});
...
app.post("/api/assessments", authenticateToken, securityHeaders, async (req, res) => {
  try {
    ...
    res.status(201).json({ assessment, code: "ASSESSMENT_CREATED" });
  } catch (error) {
    console.error("Assessment creation error:", error);
    res.status(500).json({ error: "Failed to create assessment", code: "ASSESSMENT_CREATION_FAILED" });
  }
});
...
app.post("/api/billing/checkout-session", authenticateToken, securityHeaders, async (req, res) => {
  try {
    ...
    const session = await stripeInstance.checkout.sessions.create({
      ...
      mode: 'subscription',
      success_url: `${process.env.CLIENT_URL || 'https://assesslyplatform.onrender.com'}/billing/success`,
