...
const Login = () => {
  ...
  }, [redirectMessage, from, referralCode]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Show dashboard redirect message
      ...
      if (error.isAuthError && error.details) {
        ...
        // Handle specific error cases from auth utility
        if (error.status === 401) {
        ...
        if (errorData?.detail && typeof errorData.detail === 'object') {
          ...
          if (window.gtag) {
            window.gtag('event', 'login_failed_invalid_credentials', {
              email_hash: formData.email.substring(0, 5) + '...'
            });
          }
        } 
  ...
  // Demo login handler (if applicable)
  const handleDemoLogin = () => {
    if (window.gtag) {
      window.gtag('event', 'demo_login_attempt', {
        location: 'login_page'
      });
    }
      ...
      <div className="max-w-md w-full">
        ...
        <Card className="border-2 shadow-xl">
          ...
          <CardContent>
            ...
            {config.FEATURES.DEMO_MODE && (
              <div className="mb-4">
                <Button
                  type="button"
                  variant="outline"
                  onClick={handleDemoLogin}
                  className="w-full border-dashed bg-gradient-to-r from-purple-50 to-pink-50 hover:from-purple-100 hover:to-pink-100"
                >
        ...
        <div className="mt-6 space-y-4">
          ...
          <div className="text-center space-y-2">
            ...
            {config.FEATURES.SUPPORT_EMAIL && (
              <button
                onClick={() => window.location.href = `mailto:${config.FEATURES.SUPPORT_EMAIL}`}
                className="text-xs text-gray-500 hover:text-gray-700 flex items-center justify-center mx-auto"
              >
